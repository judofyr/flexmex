...
void YGNodeListReplaceAll(const YGNodeListRef list, uint32_t count, YGNodeRef *children) {
  while (count < list->capacity) {
    list->capacity *= 2;
  }
  list->items = gYGRealloc(list->items, sizeof(YGNodeRef) * list->capacity);
  YGAssert(list->items != NULL, "Could not extend allocation for items");

  memcpy(list->items, children, sizeof(children[0])*count);
  list->count = count;
}
